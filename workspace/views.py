...
from accounts.github.rollback import rollback_file_to_commit
from accounts.github.branch import list_github_branches, list_files_in_branch
import os
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def rollback_commit_view(request):
    """
    ✅ 특정 커밋으로 파일을 되돌리는 API + DB 로그 저장
    """
    group_name = request.data.get('group_name')
    file_path = request.data.get('file_path')
    target_sha = request.data.get('target_sha')
    branch_name = request.data.get('branch_name', 'main')

    if not all([group_name, file_path, target_sha]):
        return Response({'status': 'error', 'message': '필수 인자 누락'}, status=400)

    result = rollback_file_to_commit(group_name, request.user, file_path, target_sha, branch_name)
    ...
    if result['status'] == 'success':
        ...
        # 새 커밋 SHA 확인
        url = f'https://api.github.com/repos/{github_username}/{repo}/commits?path={file_path}&sha={branch_name}'
        headers = {
            'Authorization': f'token {profile.github_token}',
            'Accept': 'application/vnd.github+json'
        }
        sha_res = requests.get(url, headers=headers)
        if sha_res.status_code == 200 and sha_res.json():
            latest_sha = sha_res.json()[0]['sha']
            UploadLog.objects.create(
                user=request.user,
                group_name=group_name,
                file_path=file_path,
                commit_message=f'{file_path} reverted to {target_sha[:7]}',
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_branch_files(request):
    """
        특정 branch를 불러와 목록 확인
    """
    group_name = request.GET.get('group_name')
    branch = request.GET.get('branch', 'main')

    branches = list_github_branches(group_name, request.user)
    files = list_files_in_branch(group_name, request.user, branch)
